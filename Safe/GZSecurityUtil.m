...
#import "GZSecurityUtil.h"
#import <Security/Security.h>
#import <CommonCrypto/CommonDigest.h>
#import <CommonCrypto/CommonCryptor.h>

@implementation GZSecurityUtil

static const Byte iv1[] = {(Byte)0x12, (Byte)0x34, (Byte)0x56, (Byte)0x78, (Byte)0x90, (Byte)0xAB, (Byte)0xCD, (Byte)0xEF};
...
+ (NSString *)encryptString:(NSString *)str publicKeyRef:(SecKeyRef)publicKeyRef{
    ...
    NSString *ret = [self base64EncodedData:data];
    return ret;
}
...
+ (SecKeyRef)addPublicKey:(NSString *)key{
    ...
    NSData *data = [self base64DecodedStringToData:key];
...
+ (NSString *)decryptString:(NSString *)str publicKey:(NSString *)privKey{
    if (!str) return nil;
    NSData *data = [[NSData alloc] initWithBase64EncodedString:str options:NSDataBase64DecodingIgnoreUnknownCharacters];
    data = [self decryptData:data publicKey:privKey];
    NSString *ret = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
    return ret;
}

+ (NSData *)decryptData:(NSData *)data publicKey:(NSString *)privKey{
    if(!data || !privKey){
        return nil;
    }
    SecKeyRef keyRef = [self addPublicKey:privKey];
    if(!keyRef){
        return nil;
    }
    return [self decryptData:data withKeyRef:keyRef];
...
+ (NSData *)base64DecodedStringToData:(NSString *)str{
...
+(NSString*)encryptWithDES:(NSString*)text andKey:(NSString*)key{
    ...
    NSUInteger dataLength = [data length] + kCCBlockSizeDES;
    ...
    if (cryptStatus == kCCSuccess) {
        ...
        plainText = [self base64EncodedData:dataTemp];
    }else{
...
+(NSString*)decryptWithDES:(NSString*)text andKey:(NSString*)key{
    if(text.length < 1 || key.length < 1) return nil;
    // 利用 GTMBase64 解碼 Base64 字串
    NSData* cipherData = [self base64DecodedStringToData:text];
