...
export default {
  ...
  data() {
    return {
      ...
      pageSize: 10,
      pageIndex: 1,
      projectId:'2206329166721191000',
      totalnum:'',
      value: "",
      isShow: true,
      isShow2: true,
      value1: -1,
      option1: [{ text: "全部", value: -1, id: 2206329166721191000 }],
      datas: [],
      urlimg:cardIcon
    };
  },
  ...
  methods: {
    ...
    add(event) {
      ...
      this.pageIndex = 1
      this.totalnum=''
      console.log('执行了---------=='+this.pageIndex)
      console.log(this.option1[event]);
      console.log(this.option1[event + 1]);
      this.projectId = this.option1[event + 1].id
      //根据下拉菜单的id拉取循环数据
      const para = {
        pageIndex: this.pageIndex,
        pageSize: this.pageSize,
    ...
    turn_detail(index, item) {
      ...
      // this.$router.push({
      //   path: '/PropertyNoticeDetail',
      //   query: { 'name': item.blockName , 'id':item.buildingName + item.roomName,'idr': item.id}
      // })
      nav.navigatorPush({
        url: "/Property_Detail",
        params: encodeURI(
