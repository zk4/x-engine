...
class ReadmeAggregator():
    ...
    def gen_dist(self):
        os.system(f"pushd {self.folder_path} && x-cli model.ts &&   yarn md && popd && rm -rdf {self.outputDir}/dist/{self.get_short_module_name()} &&  cp -fr {self.folder_path}/h5/dist {self.outputDir}/dist/{self.get_short_module_name()}")
