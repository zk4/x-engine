...
function test_同步简单数字参数() {
  let val = xengine.api("com.zkty.jsi.browser", "simpleArgNumberMethod", 1000);
  document.getElementById("debug_text").innerText = val;
}

function test_同步返回命名对象() {
  let val = xengine.api("com.zkty.jsi.browser", "namedObject", {});
  document.getElementById("debug_text").innerText =
    typeof val + ":" + val.title + "," + val.titleSize;
}

function test_同步返回匿名嵌套对象() {
  let val = xengine.api("com.zkty.jsi.browser", "nestedAnonymousObject", {});
  document.getElementById("debug_text").innerText =
    typeof val + ":" + val.a + "," + val.i.n1;
}

function test_异步返回命名对象() {
  xengine.api("com.zkty.jsi.browser", "namedObject", {}, (val) => {
    document.getElementById("debug_text").innerText =
      typeof val + ":" + val.title + "," + val.titleSize;
  });
}
...
function test_异步简单数字参数() {
  xengine.api("com.zkty.jsi.browser", "simpleArgNumberMethod", 1000, (val) => {
    document.getElementById("debug_text").innerText = val;
  });
}
...
function test_异步返回匿名嵌套对象() {
  xengine.api("com.zkty.jsi.browser", "nestedAnonymousObject", {}, (val) => {
    document.getElementById("debug_text").innerText =
      typeof val + ":" + val.a + "," + val.i.n1;
  });
}
...
function test_complex_sync() {
  val = xengine.api("com.zkty.jsi.browser", "complexAnoymousRetWithAnoymousArgs", {
    name: "zk",
    age: 12,
    houses: [
      {
