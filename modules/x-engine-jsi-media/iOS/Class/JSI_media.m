...
#import <x-engine-native-media/iMedia.h>
...
// 保存图片到相册
- (void)_saveImageToPhotoAlbum:(_saveImageToPhotoAlbum_com_zkty_jsi_media_1_DTO *)dto complete:(void (^)(_saveImageToPhotoAlbum_com_zkty_jsi_media_0_DTO *, BOOL))completionHandler {
    ...
    if (image != nil) {
        [self.media saveImageToPhotoAlbumWithImage:image result:^(UIImage *image, NSError *error) {
            _saveImageToPhotoAlbum_com_zkty_jsi_media_0_DTO *cb = [_saveImageToPhotoAlbum_com_zkty_jsi_media_0_DTO new];
            if (error) {
                cb.status = -1;
...
// 打开picker
- (void)_openImagePicker:(_openImagePicker_com_zkty_jsi_media_2_DTO *)dto complete:(void (^)(_openImagePicker_com_zkty_jsi_media_0_DTO *, BOOL))completionHandler {
    // 1. 转换 jsi 参数到 native 需要的参数
    MediaParamsDTO* nativeDto = [MediaParamsDTO new];
    ...
    [self.media openImagePicker:nativeDto result:^(NSArray<UIImage *> *images, NSArray *assets) {
        _openImagePicker_com_zkty_jsi_media_0_DTO* jsiRet = [_openImagePicker_com_zkty_jsi_media_0_DTO new];
        jsiRet.status = 0;
        jsiRet.msg = @"";
        jsiRet.data = [NSMutableArray<_openImagePicker_com_zkty_jsi_media_1_DTO *><_openImagePicker_com_zkty_jsi_media_1_DTO> array];
        if (assets) {// 相册
            NSArray *array = [self getAlbumWithAssets:assets];
            [array enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                _openImagePicker_com_zkty_jsi_media_1_DTO * imgObj = [_openImagePicker_com_zkty_jsi_media_1_DTO new];
                imgObj.imgID = obj[@"id"];
...
// 上传图片
- (void)_uploadImage:(_uploadImage_com_zkty_jsi_media_1_DTO *)dto complete:(void (^)(_uploadImage_com_zkty_jsi_media_0_DTO *, BOOL))completionHandler {
    ...
    [dto.imgIds enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        [self.media uploadImageWithUrl:dto.url withImage:image withImageName:imageName success:^(NSDictionary *dict) {
            _uploadImage_com_zkty_jsi_media_0_DTO *h5CallBack = [_uploadImage_com_zkty_jsi_media_0_DTO new];
            h5CallBack.status = 0;
