//
//  ZKTY_TZImagePickerController.m
//  ZKTY_TZImagePickerController
//
//  Created by 谭真 on 15/12/24.
//  Copyright © 2015年 谭真. All rights reserved.
//  version 3.5.4 - 2020.11.25
//  更多信息，请前往项目的github地址：https://github.com/banchichen/ZKTY_TZImagePickerController
...
@interface ZKTY_TZImagePickerController () {
    NSTimer *_timer;
    UILabel *_tipLabel;
...
/// Default is 4, Use in photos collectionView in ZKTY_TZPhotoPickerController
/// 默认4列, ZKTY_TZPhotoPickerController中的照片collectionView
@property (nonatomic, assign) NSInteger columnNumber;
@property (nonatomic, assign) NSInteger HUDTimeoutCount; ///< 超时隐藏HUD计数
@end

@implementation ZKTY_TZImagePickerController
...
- (void)viewDidLoad {
    ...
    [ZKTY_TZImageManager manager].shouldFixOrientation = NO;
...
- (void)configBarButtonItemAppearance {
    ...
    if (@available(iOS 9, *)) {
        barItem = [UIBarButtonItem appearanceWhenContainedInInstancesOfClasses:@[[ZKTY_TZImagePickerController class]]];
    } else {
        barItem = [UIBarButtonItem appearanceWhenContainedIn:[ZKTY_TZImagePickerController class], nil];
    }
    NSMutableDictionary *textAttrs = [NSMutableDictionary dictionary];
...
- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount delegate:(id<ZKTY_TZImagePickerControllerDelegate>)delegate {
    return [self initWithMaxImagesCount:maxImagesCount columnNumber:4 delegate:delegate pushPhotoPickerVc:YES];
}

- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount columnNumber:(NSInteger)columnNumber delegate:(id<ZKTY_TZImagePickerControllerDelegate>)delegate {
    return [self initWithMaxImagesCount:maxImagesCount columnNumber:columnNumber delegate:delegate pushPhotoPickerVc:YES];
}

- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount columnNumber:(NSInteger)columnNumber delegate:(id<ZKTY_TZImagePickerControllerDelegate>)delegate pushPhotoPickerVc:(BOOL)pushPhotoPickerVc {
    _pushPhotoPickerVc = pushPhotoPickerVc;
    ZKTY_TZAlbumPickerController *albumPickerVc = [[ZKTY_TZAlbumPickerController alloc] init];
    ...
    if (self) {
        ...
        if (![[ZKTY_TZImageManager manager] authorizationStatusAuthorized]) {
...
- (instancetype)initWithSelectedAssets:(NSMutableArray *)selectedAssets selectedPhotos:(NSMutableArray *)selectedPhotos index:(NSInteger)index{
    ZKTY_TZPhotoPreviewController *previewVc = [[ZKTY_TZPhotoPreviewController alloc] init];
...
- (void)setShowSelectedIndex:(BOOL)showSelectedIndex {
    ...
    [ZKTY_TZImagePickerConfig sharedInstance].showSelectedIndex = showSelectedIndex;
}

- (void)setShowPhotoCannotSelectLayer:(BOOL)showPhotoCannotSelectLayer {
    _showPhotoCannotSelectLayer = showPhotoCannotSelectLayer;
    [ZKTY_TZImagePickerConfig sharedInstance].showPhotoCannotSelectLayer = showPhotoCannotSelectLayer;
}

- (void)setNotScaleImage:(BOOL)notScaleImage {
    _notScaleImage = notScaleImage;
    [ZKTY_TZImagePickerConfig sharedInstance].notScaleImage = notScaleImage;
}

- (void)setNeedFixComposition:(BOOL)needFixComposition {
    _needFixComposition = needFixComposition;
    [ZKTY_TZImagePickerConfig sharedInstance].needFixComposition = needFixComposition;
}
...
- (void)observeAuthrizationStatusChange {
    ...
    if ([[ZKTY_TZImageManager manager] authorizationStatusAuthorized]) {
        ...
        ZKTY_TZAlbumPickerController *albumPickerVc = (ZKTY_TZAlbumPickerController *)self.visibleViewController;
        if ([albumPickerVc isKindOfClass:[ZKTY_TZAlbumPickerController class]]) {
...
- (void)pushPhotoPickerVc {
    ...
    if (!_didPushPhotoPickerVc && _pushPhotoPickerVc) {
        ZKTY_TZPhotoPickerController *photoPickerVc = [[ZKTY_TZPhotoPickerController alloc] init];
        photoPickerVc.isFirstAppear = YES;
        photoPickerVc.columnNumber = self.columnNumber;
        [[ZKTY_TZImageManager manager] getCameraRollAlbumWithFetchAssets:NO completion:^(ZKTY_TZAlbumModel *model) {
...
- (void)setPickerDelegate:(id<ZKTY_TZImagePickerControllerDelegate>)pickerDelegate {
    _pickerDelegate = pickerDelegate;
    [ZKTY_TZImageManager manager].pickerDelegate = pickerDelegate;
}
...
- (void)setPhotoPreviewMaxWidth:(CGFloat)photoPreviewMaxWidth {
    ...
    [ZKTY_TZImageManager manager].photoPreviewMaxWidth = _photoPreviewMaxWidth;
}

- (void)setPhotoWidth:(CGFloat)photoWidth {
    _photoWidth = photoWidth;
    [ZKTY_TZImageManager manager].photoWidth = photoWidth;
}
...
- (void)setSelectedAssets:(NSMutableArray *)selectedAssets {
    ...
    for (PHAsset *asset in selectedAssets) {
        ZKTY_TZAssetModel *model = [ZKTY_TZAssetModel modelWithAsset:asset type:[[ZKTY_TZImageManager manager] getAssetType:asset]];
...
- (void)setAllowPickingImage:(BOOL)allowPickingImage {
    ...
    [ZKTY_TZImagePickerConfig sharedInstance].allowPickingImage = allowPickingImage;
...
- (void)setAllowPickingVideo:(BOOL)allowPickingVideo {
    ...
    [ZKTY_TZImagePickerConfig sharedInstance].allowPickingVideo = allowPickingVideo;
...
- (void)setPreferredLanguage:(NSString *)preferredLanguage {
    ...
    [ZKTY_TZImagePickerConfig sharedInstance].preferredLanguage = preferredLanguage;
    [self configDefaultBtnTitle];
}

- (void)setLanguageBundle:(NSBundle *)languageBundle {
    _languageBundle = languageBundle;
    [ZKTY_TZImagePickerConfig sharedInstance].languageBundle = languageBundle;
    [self configDefaultBtnTitle];
}
...
- (void)setSortAscendingByModificationDate:(BOOL)sortAscendingByModificationDate {
    ...
    [ZKTY_TZImageManager manager].sortAscendingByModificationDate = sortAscendingByModificationDate;
}
...
- (void)addSelectedModel:(ZKTY_TZAssetModel *)model {
    [_selectedModels addObject:model];
    [_selectedAssetIds addObject:model.asset.localIdentifier];
}

- (void)removeSelectedModel:(ZKTY_TZAssetModel *)model {
...
@interface ZKTY_TZAlbumPickerController ()<UITableViewDataSource, UITableViewDelegate, PHPhotoLibraryChangeObserver> {
    UITableView *_tableView;
}
@property (nonatomic, strong) NSMutableArray *albumArr;
@end

@implementation ZKTY_TZAlbumPickerController
...
- (void)viewDidLoad {
    ...
    [ZKTY_TZCommonTools configBarButtonItem:cancelItem tzImagePickerVc:imagePickerVc];
...
- (void)configTableView {
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        [[ZKTY_TZImageManager manager] getAllAlbumsWithFetchAssets:!self.isFirstAppear completion:^(NSArray<ZKTY_TZAlbumModel *> *models) {
            dispatch_async(dispatch_get_main_queue(), ^{
                self->_albumArr = [NSMutableArray arrayWithArray:models];
                for (ZKTY_TZAlbumModel *albumModel in self->_albumArr) {
                ...
                if (!self->_tableView) {
                    ...
                    [self->_tableView registerClass:[ZKTY_TZAlbumCell class] forCellReuseIdentifier:@"ZKTY_TZAlbumCell"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ZKTY_TZAlbumCell *cell = [tableView dequeueReusableCellWithIdentifier:@"ZKTY_TZAlbumCell"];
    if (@available(iOS 13.0, *)) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ZKTY_TZPhotoPickerController *photoPickerVc = [[ZKTY_TZPhotoPickerController alloc] init];
    photoPickerVc.columnNumber = self.columnNumber;
    ZKTY_TZAlbumModel *model = _albumArr[indexPath.row];
    photoPickerVc.model = model;
...
@implementation ZKTY_TZCommonTools
...
+ (void)configBarButtonItem:(UIBarButtonItem *)item tzImagePickerVc:(ZKTY_TZImagePickerController *)tzImagePickerVc {
...
@implementation ZKTY_TZImagePickerConfig

+ (instancetype)sharedInstance {
    static dispatch_once_t onceToken;
    static ZKTY_TZImagePickerConfig *config = nil;
    dispatch_once(&onceToken, ^{
        if (config == nil) {
            config = [[ZKTY_TZImagePickerConfig alloc] init];
