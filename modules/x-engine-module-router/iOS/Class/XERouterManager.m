...
+(instancetype)instance{
    ...
    static XERouterManager *manage;
    dispatch_once(&onceToken, ^{
        manage = [[XERouterManager alloc] init]; 
    });
    return manage;
}
...
-(void)routerToTarget:(NSString *)type withUri:(NSString *)uri withPath:(NSString *)path withArgs:(NSDictionary *)args withVersion:(long)version{
    [self routerToTarget:type withUri:uri withPath:path withArgs:args withVersion:version withHiddenNavbar:NO];
}

-(void)routerToTarget:(NSString *)type
    ...
    BOOL isContinue = YES;
    if(self.delegate && [self.delegate respondsToSelector:@selector(preIsRouterToTarget:withUri:withPath:withArgs:withVersion:withHiddenNavbar:)]){
        isContinue = [self.delegate preIsRouterToTarget:type withUri:uri withPath:path withArgs:args withVersion:version withHiddenNavbar:isHidden];
    }
    if(isContinue){
        if(uri.length == 0){
            return;
...
+(void)routerToTarget:(NSString *)type
             ...
             withArgs:(NSDictionary *)args
          withVersion:(long)version{
    [[XERouterManager instance] routerToTarget:type
                                       withUri:uri
                                      withPath:path
                                      withArgs:args
                                   withVersion:version
