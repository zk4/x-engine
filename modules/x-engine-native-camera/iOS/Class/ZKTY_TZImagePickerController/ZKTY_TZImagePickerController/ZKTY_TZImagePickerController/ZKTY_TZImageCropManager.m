...
@implementation ZKTY_TZImageCropManager
...
+ (UIImage *)cropImageView:(UIImageView *)imageView toRect:(CGRect)rect zoomScale:(double)zoomScale containerView:(UIView *)containerView {
    ...
    cropedImage = [[ZKTY_TZImageManager manager] fixOrientation:cropedImage];
...
@implementation UIImage (ZKTY_TZGif)
...
+ (UIImage *)sd_tz_animatedGIFWithData:(NSData *)data {
    ...
    if (count <= 1) {
        ...
        NSInteger maxCount = [ZKTY_TZImagePickerConfig sharedInstance].gifPreviewMaxImagesCount ?: 50;
