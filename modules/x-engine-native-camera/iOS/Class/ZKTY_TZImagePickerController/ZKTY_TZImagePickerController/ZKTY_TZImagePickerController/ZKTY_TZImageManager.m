...
static ZKTY_TZImageManager *manager;
...
+ (instancetype)manager {
    dispatch_once(&onceToken, ^{
        ...
        [manager configZKTY_TZScreenWidth];
    });
    return manager;
}
...
- (void)setPhotoWidth:(CGFloat)photoWidth {
    ...
    ZKTY_TZScreenWidth = photoWidth / 2;
}

- (void)setColumnNumber:(NSInteger)columnNumber {
    [self configZKTY_TZScreenWidth];
...
- (void)configZKTY_TZScreenWidth {
    ZKTY_TZScreenWidth = [UIScreen mainScreen].bounds.size.width;
    // 测试发现，如果scale在plus真机上取到3.0，内存会增大特别多。故这里写死成2.0
    ZKTY_TZScreenScale = 2.0;
    if (ZKTY_TZScreenWidth > 700) {
        ZKTY_TZScreenScale = 1.5;
    }
}
...
- (void)getCameraRollAlbum:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage needFetchAssets:(BOOL)needFetchAssets completion:(void (^)(ZKTY_TZAlbumModel *model))completion {
    ZKTY_TZImagePickerConfig *config = [ZKTY_TZImagePickerConfig sharedInstance];
...
/// Get Album 获得相册/相册数组
- (void)getCameraRollAlbumWithFetchAssets:(BOOL)needFetchAssets completion:(void (^)(ZKTY_TZAlbumModel *model))completion {
    __block ZKTY_TZAlbumModel *model;
    ZKTY_TZImagePickerConfig *config = [ZKTY_TZImagePickerConfig sharedInstance];
...
- (void)getAllAlbums:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage needFetchAssets:(BOOL)needFetchAssets completion:(void (^)(NSArray<ZKTY_TZAlbumModel *> *))completion {
    ZKTY_TZImagePickerConfig *config = [ZKTY_TZImagePickerConfig sharedInstance];
    config.allowPickingVideo = allowPickingVideo;
    config.allowPickingImage = allowPickingImage;
...
- (void)getAllAlbumsWithFetchAssets:(BOOL)needFetchAssets completion:(void (^)(NSArray<ZKTY_TZAlbumModel *> *))completion {
    ZKTY_TZImagePickerConfig *config = [ZKTY_TZImagePickerConfig sharedInstance];
...
/// Get Assets 获得照片数组
- (void)getAssetsFromFetchResult:(PHFetchResult *)result allowPickingVideo:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage completion:(void (^)(NSArray<ZKTY_TZAssetModel *> *))completion {
    ZKTY_TZImagePickerConfig *config = [ZKTY_TZImagePickerConfig sharedInstance];
    config.allowPickingVideo = allowPickingVideo;
    config.allowPickingImage = allowPickingImage;
...
- (void)getAssetsFromFetchResult:(PHFetchResult *)result completion:(void (^)(NSArray<ZKTY_TZAssetModel *> *))completion {
    ZKTY_TZImagePickerConfig *config = [ZKTY_TZImagePickerConfig sharedInstance];
    NSMutableArray *photoArr = [NSMutableArray array];
    [result enumerateObjectsUsingBlock:^(PHAsset *asset, NSUInteger idx, BOOL * _Nonnull stop) {
        ZKTY_TZAssetModel *model = [self assetModelWithAsset:asset allowPickingVideo:config.allowPickingVideo allowPickingImage:config.allowPickingImage];
        if (model) {
...
- (void)getAssetFromFetchResult:(PHFetchResult *)result atIndex:(NSInteger)index allowPickingVideo:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage completion:(void (^)(ZKTY_TZAssetModel *))completion {
    ZKTY_TZImagePickerConfig *config = [ZKTY_TZImagePickerConfig sharedInstance];
    config.allowPickingVideo = allowPickingVideo;
    config.allowPickingImage = allowPickingImage;
...
- (void)getAssetFromFetchResult:(PHFetchResult *)result atIndex:(NSInteger)index completion:(void (^)(ZKTY_TZAssetModel *))completion {
    PHAsset *asset;
    @try {
        asset = result[index];
    }
...
- (ZKTY_TZAssetModel *)assetModelWithAsset:(PHAsset *)asset allowPickingVideo:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage {
    BOOL canSelect = YES;
    ...
    ZKTY_TZAssetModelMediaType type = [self getAssetType:asset];
    if (!allowPickingVideo && type == ZKTY_TZAssetModelMediaTypeVideo) return nil;
    if (!allowPickingImage && type == ZKTY_TZAssetModelMediaTypePhoto) return nil;
    if (!allowPickingImage && type == ZKTY_TZAssetModelMediaTypePhotoGif) return nil;
    ...
    NSString *timeLength = type == ZKTY_TZAssetModelMediaTypeVideo ? [NSString stringWithFormat:@"%0.0f",phAsset.duration] : @"";
    timeLength = [self getNewTimeFromDurationSecond:timeLength.integerValue];
    model = [ZKTY_TZAssetModel modelWithAsset:asset type:type timeLength:timeLength];
    return model;
}

- (ZKTY_TZAssetModelMediaType)getAssetType:(PHAsset *)asset {
    ZKTY_TZAssetModelMediaType type = ZKTY_TZAssetModelMediaTypePhoto;
    PHAsset *phAsset = (PHAsset *)asset;
    if (phAsset.mediaType == PHAssetMediaTypeVideo)      type = ZKTY_TZAssetModelMediaTypeVideo;
    else if (phAsset.mediaType == PHAssetMediaTypeAudio) type = ZKTY_TZAssetModelMediaTypeAudio;
    else if (phAsset.mediaType == PHAssetMediaTypeImage) {
        if (@available(iOS 9.1, *)) {
            // if (asset.mediaSubtypes == PHAssetMediaSubtypePhotoLive) type = ZKTY_TZAssetModelMediaTypeLivePhoto;
        }
        // Gif
        if ([[phAsset valueForKey:@"filename"] hasSuffix:@"GIF"]) {
            type = ZKTY_TZAssetModelMediaTypePhotoGif;
        }
    }
    return type;
}
...
- (void)getPhotosBytesWithArray:(NSArray *)photos completion:(void (^)(NSString *totalBytes))completion {
    ...
    for (NSInteger i = 0; i < photos.count; i++) {
        ZKTY_TZAssetModel *model = photos[i];
        PHImageRequestOptions *options = [[PHImageRequestOptions alloc] init];
        options.resizeMode = PHImageRequestOptionsResizeModeFast;
        options.networkAccessAllowed = YES;
        if (model.type == ZKTY_TZAssetModelMediaTypePhotoGif) {
        ...
        [[PHImageManager defaultManager] requestImageDataForAsset:model.asset options:options resultHandler:^(NSData *imageData, NSString *dataUTI, UIImageOrientation orientation, NSDictionary *info) {
            if (model.type != ZKTY_TZAssetModelMediaTypeVideo) dataLength += imageData.length;
            assetCount ++;
...
- (PHImageRequestID)getPhotoWithAsset:(PHAsset *)asset completion:(void (^)(UIImage *, NSDictionary *, BOOL isDegraded))completion {
    CGFloat fullScreenWidth = ZKTY_TZScreenWidth;
...
- (PHImageRequestID)getPhotoWithAsset:(PHAsset *)asset photoWidth:(CGFloat)photoWidth completion:(void (^)(UIImage *photo,NSDictionary *info,BOOL isDegraded))completion progressHandler:(void (^)(double progress, NSError *error, BOOL *stop, NSDictionary *info))progressHandler networkAccessAllowed:(BOOL)networkAccessAllowed {
    ...
    if (photoWidth < ZKTY_TZScreenWidth && photoWidth < _photoPreviewMaxWidth) {
        ...
        CGFloat pixelWidth = photoWidth * ZKTY_TZScreenScale;
        // 超宽图片
        if (aspectRatio > 1.8) {
...
/// Get postImage / 获取封面图
- (PHImageRequestID)getPostImageWithAlbumModel:(ZKTY_TZAlbumModel *)model completion:(void (^)(UIImage *))completion {
    ...
    return [[ZKTY_TZImageManager manager] getPhotoWithAsset:asset photoWidth:80 completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
...
- (void)startExportVideoWithVideoAsset:(AVURLAsset *)videoAsset presetName:(NSString *)presetName success:(void (^)(NSString *outputPath))success failure:(void (^)(NSString *errorMessage, NSError *error))failure {
    ...
    if ([presets containsObject:presetName]) {
        ...
        if ([ZKTY_TZImagePickerConfig sharedInstance].needFixComposition) {
...
- (ZKTY_TZAssetModel *)createModelWithAsset:(PHAsset *)asset {
    ZKTY_TZAssetModelMediaType type = [[ZKTY_TZImageManager manager] getAssetType:asset];
    NSString *timeLength = type == ZKTY_TZAssetModelMediaTypeVideo ? [NSString stringWithFormat:@"%0.0f",asset.duration] : @"";
    timeLength = [[ZKTY_TZImageManager manager] getNewTimeFromDurationSecond:timeLength.integerValue];
    ZKTY_TZAssetModel *model = [ZKTY_TZAssetModel modelWithAsset:asset type:type timeLength:timeLength];
    return model;
}
...
//@implementation ZKTY_TZSortDescriptor
//
//- (id)reversedSortDescriptor {
//    return [NSNumber numberWithBool:![ZKTY_TZImageManager manager].sortAscendingByModificationDate];
//}
//
//@end
