...
@implementation ZKTY_TZPhotoPreviewCell

- (void)configSubviews {
    self.previewView = [[ZKTY_TZPhotoPreviewView alloc] initWithFrame:CGRectZero];
...
- (void)setModel:(ZKTY_TZAssetModel *)model {
...
@interface ZKTY_TZPhotoPreviewView ()<UIScrollViewDelegate>
@property (assign, nonatomic) BOOL isRequestingGIF;
@end

@implementation ZKTY_TZPhotoPreviewView
...
- (void)configProgressView {
    _progressView = [[ZKTY_TZProgressView alloc] init];
...
- (void)setModel:(ZKTY_TZAssetModel *)model {
    ...
    if (model.type == ZKTY_TZAssetModelMediaTypePhotoGif) {
        // 先显示缩略图
        [[ZKTY_TZImageManager manager] getPhotoWithAsset:model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
            if (photo) {
            ...
            [[ZKTY_TZImageManager manager] getOriginalPhotoDataWithAsset:model.asset progressHandler:^(double progress, NSError *error, BOOL *stop, NSDictionary *info) {
                progress = progress > 0.02 ? progress : 0.02;
                dispatch_async(dispatch_get_main_queue(), ^{
                    BOOL iCloudSyncFailed = [ZKTY_TZCommonTools isICloudSyncError:error];
...
#ifdef DEBUG
                NSLog(@"[ZKTY_TZImagePickerController] getOriginalPhotoDataWithAsset:%f error:%@", progress, error);
#endif
            } completion:^(NSData *data, NSDictionary *info, BOOL isDegraded) {
                if (!isDegraded) {
                    self.isRequestingGIF = NO;
                    self.progressView.hidden = YES;
                    if ([ZKTY_TZImagePickerConfig sharedInstance].gifImagePlayBlock) {
                        [ZKTY_TZImagePickerConfig sharedInstance].gifImagePlayBlock(self, self.imageView, data, info);
                    } else {
...
- (void)setAsset:(PHAsset *)asset {
    ...
    _asset = asset;
    self.imageRequestID = [[ZKTY_TZImageManager manager] getPhotoWithAsset:asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
        BOOL iCloudSyncFailed = !photo && [ZKTY_TZCommonTools isICloudSyncError:info[PHImageErrorKey]];
        self.iCloudErrorLabel.hidden = !iCloudSyncFailed;
        self.iCloudErrorIcon.hidden = !iCloudSyncFailed;
        if (self.iCloudSyncFailedHandle) {
...
- (void)layoutSubviews {
    ...
    _iCloudErrorIcon.frame = CGRectMake(20, [ZKTY_TZCommonTools tz_statusBarHeight] + 44 + 10, 28, 28);
    _iCloudErrorLabel.frame = CGRectMake(53, [ZKTY_TZCommonTools tz_statusBarHeight] + 44 + 10, self.tz_width - 63, 28);
}
...
@implementation ZKTY_TZVideoPreviewCell
...
- (void)configMoviePlayer {
    ...
    if (self.model && self.model.asset) {
        [[ZKTY_TZImageManager manager] getPhotoWithAsset:self.model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
            BOOL iCloudSyncFailed = !photo && [ZKTY_TZCommonTools isICloudSyncError:info[PHImageErrorKey]];
            self.iCloudErrorLabel.hidden = !iCloudSyncFailed;
            self.iCloudErrorIcon.hidden = !iCloudSyncFailed;
            if (self.iCloudSyncFailedHandle) {
        ...
        [[ZKTY_TZImageManager manager] getVideoWithAsset:self.model.asset completion:^(AVPlayerItem *playerItem, NSDictionary *info) {
            dispatch_async(dispatch_get_main_queue(), ^{
                BOOL iCloudSyncFailed = !playerItem && [ZKTY_TZCommonTools isICloudSyncError:info[PHImageErrorKey]];
                self.iCloudErrorLabel.hidden = !iCloudSyncFailed;
                self.iCloudErrorIcon.hidden = !iCloudSyncFailed;
                if (self.iCloudSyncFailedHandle) {
                    self.iCloudSyncFailedHandle(self.model.asset, iCloudSyncFailed);
...
- (void)playButtonClick {
    ...
    if (_player.rate == 0.0f) {
        [[NSNotificationCenter defaultCenter] postNotificationName:@"ZKTY_TZ_VIDEO_PLAY_NOTIFICATION" object:_player];
...
@implementation ZKTY_TZGifPreviewCell
