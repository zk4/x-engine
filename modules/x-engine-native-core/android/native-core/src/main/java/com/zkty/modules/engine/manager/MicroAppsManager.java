...
import static com.zkty.modules.engine.utils.Utils.isNumeric;
...
public class MicroAppsManager {
    ...
    private static MicroAppsManager INSTANCE;
                    ...
                    public void run() {
                        ...
                        if (apps.size() > 0) {
                            ...
                            for (int i = 0; i < apps.size(); i++) {                                 //安装微应用
                                ...
                                try {
                                    if (!MicroAppsManager.getInstance().isMicroAppExit(app, version)) {
                                        Log.d(TAG, "start install apps:" + app + "." + version);
                                        InputStream inputStream = context.getAssets().open(ASSET_APPS_DIR + "/" + apps.get(i));
                                        MicroAppsManager.getInstance().installFormAsset(inputStream, app, version);
                                    }
                                } catch (IOException e) {
     ...
     * @return
     */
    public boolean isMicroAppExit(String microAppId) {
        boolean exit = false;
     ...
     * @return
     */
    public boolean isMicroAppExit(String microAppId, String version) {
        boolean exit = false;
        if (!TextUtils.isEmpty(microAppId) && !TextUtils.isEmpty(version)) {
            File webAppDir = getWebAppRoot();
            if (webAppDir.exists()) {                                           //微应用集合目录是否存在
                File appDir = new File(webAppDir, microAppId);
                if (appDir.exists()) {                                          //指定的微应用是否存在
    ...
    public String getMicroAppPath(String microAppId) {
        ...
        if (!TextUtils.isEmpty(microAppId)) {
            if (isMicroAppExit(microAppId)) {
                path = getMicroAppPath(microAppId, String.valueOf(getHighMicroAppVersionCode(microAppId)));
            }
        }
        return path;
    }
     ...
     * @return
     */
    public String getMicroAppPath(String microAppId, String version) {
        String path = null;
        if (!TextUtils.isEmpty(microAppId) && !TextUtils.isEmpty(version)) {
            if (isMicroAppExit(microAppId)) {
     ...
     * @return
     */
    public ArrayList<String> listMicroAppVersions(String microAppId) {
        ArrayList<String> apps = new ArrayList<>();
        if (isMicroAppExit(microAppId)) {
     ...
     * @return -1:不存在
     */
    public int getHighMicroAppVersionCode(String microAppId) {
        int code = -1;
     ...
     * @return
     */
    public String saveApp(InputStream inputStream, String microAppId, String version) {
        String path = null;
        if (inputStream != null && !TextUtils.isEmpty(microAppId) && !TextUtils.isEmpty(version)) {
    ...
    public void uninstallApp(String microAppId) {
        File app = new File(MicroAppsManager.getInstance().getWebAppRoot(), microAppId);
        if (app.exists()) {
     ...
     * @return
     */
    public boolean installFormAsset(InputStream asset, String microAppId, String version) {
        boolean success = false;
