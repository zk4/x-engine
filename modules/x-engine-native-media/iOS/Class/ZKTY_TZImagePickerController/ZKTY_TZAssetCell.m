...
@implementation ZKTY_TZAssetCell

- (instancetype)initWithFrame:(CGRect)frame {
    self = [super initWithFrame:frame];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(reload:) name:@"ZKTY_TZ_PHOTO_PICKER_RELOAD_NOTIFICATION" object:nil];
    return self;
}
...
- (void)setModel:(ZKTY_TZAssetModel *)model {
    ...
    int32_t imageRequestID = [[ZKTY_TZImageManager manager] getPhotoWithAsset:model.asset photoWidth:self.tz_width completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
...
- (void)setShowSelectBtn:(BOOL)showSelectBtn {
    ...
    BOOL selectable = [[ZKTY_TZImageManager manager] isPhotoSelectableWithAsset:self.model.asset];
...
- (void)setType:(ZKTY_TZAssetCellType)type {
    _type = type;
    if (type == ZKTY_TZAssetCellTypePhoto || type == ZKTY_TZAssetCellTypeLivePhoto || (type == ZKTY_TZAssetCellTypePhotoGif && !self.allowPickingGif) || self.allowPickingMultipleVideo) {
    ...
    if (type == ZKTY_TZAssetCellTypeVideo) {
    ...
    } else if (type == ZKTY_TZAssetCellTypePhotoGif && self.allowPickingGif) {
        self.bottomView.hidden = NO;
...
- (void)selectPhotoButtonClick:(UIButton *)sender {
    ...
    if (sender.isSelected) {
        [UIView showOscillatoryAnimationWithLayer:_selectImageView.layer type:ZKTY_TZOscillatoryAnimationToBigger];
        // 用户选中了该图片，提前获取一下大图
        [self requestBigImage];
    } else { // 取消选中，取消大图的获取
        [self cancelBigImageRequest];
...
- (void)requestBigImage {
    ...
    _bigImageRequestID = [[ZKTY_TZImageManager manager] requestImageDataForAsset:_model.asset completion:^(NSData *imageData, NSString *dataUTI, UIImageOrientation orientation, NSDictionary *info) {
        BOOL iCloudSyncFailed = !imageData && [ZKTY_TZCommonTools isICloudSyncError:info[PHImageErrorKey]];
    ...
    if (_model.type == ZKTY_TZAssetCellTypeVideo) {
        [[ZKTY_TZImageManager manager] getVideoWithAsset:_model.asset completion:^(AVPlayerItem *playerItem, NSDictionary *info) {
...
- (void)reload:(NSNotification *)noti {
    ZKTY_TZImagePickerController *tzImagePickerVc = (ZKTY_TZImagePickerController *)noti.object;
...
- (ZKTY_TZProgressView *)progressView {
...
@interface ZKTY_TZAlbumCell ()
@property (weak, nonatomic) UIImageView *posterImageView;
@property (weak, nonatomic) UILabel *titleLabel;
@end

@implementation ZKTY_TZAlbumCell
...
- (void)setModel:(ZKTY_TZAlbumModel *)model {
    _model = model;
    ...
    [[ZKTY_TZImageManager manager] getPostImageWithAlbumModel:model completion:^(UIImage *postImage) {
...
@implementation ZKTY_TZAssetCameraCell
