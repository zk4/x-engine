//
//  ZKTY_TZPhotoPickerController.m
...
#import "ZKTY_TZPhotoPickerController.h"
#import "ZKTY_TZImagePickerController.h"
#import "ZKTY_TZPhotoPreviewController.h"
#import "ZKTY_TZAssetCell.h"
#import "ZKTY_TZAssetModel.h"
#import "ZKTY_UIView+Layout.h"
#import "ZKTY_TZImageManager.h"
#import "ZKTY_TZVideoPlayerController.h"
#import "ZKTY_TZGifPhotoPreviewController.h"
#import "ZKTY_TZLocationManager.h"
#import <MobileCoreServices/MobileCoreServices.h>
#import "ZKTY_TZImageRequestOperation.h"

@interface ZKTY_TZPhotoPickerController ()<UICollectionViewDataSource,UICollectionViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate, PHPhotoLibraryChangeObserver> {
...
@property (nonatomic, strong) ZKTY_TZCollectionView *collectionView;
...
- (UIImagePickerController *)imagePickerVc {
    if (_imagePickerVc == nil) {
        ...
        if (@available(iOS 9, *)) {
            tzBarItem = [UIBarButtonItem appearanceWhenContainedInInstancesOfClasses:@[[ZKTY_TZImagePickerController class]]];
            BarItem = [UIBarButtonItem appearanceWhenContainedInInstancesOfClasses:@[[UIImagePickerController class]]];
        } else {
            tzBarItem = [UIBarButtonItem appearanceWhenContainedIn:[ZKTY_TZImagePickerController class], nil];
...
- (void)viewDidLoad {
    ...
    [ZKTY_TZCommonTools configBarButtonItem:cancelItem tzImagePickerVc:tzImagePickerVc];
    ...
    if (tzImagePickerVc.navLeftBarButtonSettingBlock) {
        ...
        [ZKTY_TZCommonTools configBarButtonItem:backItem tzImagePickerVc:tzImagePickerVc];
...
- (void)fetchAssetModels {
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        if (!tzImagePickerVc.sortAscendingByModificationDate && self->_isFirstAppear && self->_model.isCameraRoll) {
            [[ZKTY_TZImageManager manager] getCameraRollAlbumWithFetchAssets:YES completion:^(ZKTY_TZAlbumModel *model) {
                self->_model = model;
                self->_models = [NSMutableArray arrayWithArray:self->_model.models];
                [self initSubviews];
            }];
        } else if (self->_showTakePhotoBtn || self->_isFirstAppear || !self.model.models) {
            [[ZKTY_TZImageManager manager] getAssetsFromFetchResult:self->_model.result completion:^(NSArray<ZKTY_TZAssetModel *> *models) {
...
- (void)configCollectionView {
    if (!_collectionView) {
        ...
        _collectionView = [[ZKTY_TZCollectionView alloc] initWithFrame:CGRectZero collectionViewLayout:_layout];
        if (@available(iOS 13.0, *)) {
...
- (void)configBottomToolBar {
    ...
    if (tzImagePickerVc.allowPickingOriginalPhoto) {
        ...
        _originalPhotoButton.imageEdgeInsets = UIEdgeInsetsMake(0, [ZKTY_TZCommonTools tz_isRightToLeftLayout] ? 10 : -10, 0, 0);
...
- (void)viewDidLayoutSubviews {
    ...
    CGFloat toolBarHeight = 50 + [ZKTY_TZCommonTools tz_safeAreaInsets].bottom;
    if (self.navigationController.navigationBar.isTranslucent) {
        top = naviBarHeight;
        if (!isStatusBarHidden && isFullScreen) top += [ZKTY_TZCommonTools tz_statusBarHeight];
    ...
    if (!self.navigationController.navigationBar.isHidden) {
        ...
        CGFloat navigationHeight = naviBarHeight + [ZKTY_TZCommonTools tz_statusBarHeight];
    ...
    [ZKTY_TZImageManager manager].columnNumber = [ZKTY_TZImageManager manager].columnNumber;
    [ZKTY_TZImageManager manager].photoWidth = tzImagePickerVc.photoWidth;
...
- (void)previewButtonClick {
    ZKTY_TZPhotoPreviewController *photoPreviewVc = [[ZKTY_TZPhotoPreviewController alloc] init];
...
- (void)doneButtonClick {
    ...
    if (tzImagePickerVc.onlyReturnAsset) { // not fetch image
        for (NSInteger i = 0; i < tzImagePickerVc.selectedModels.count; i++) {
            ZKTY_TZAssetModel *model = tzImagePickerVc.selectedModels[i];
        ...
        [ZKTY_TZImageManager manager].shouldFixOrientation = YES;
        __block UIAlertController *alertView;
        for (NSInteger i = 0; i < tzImagePickerVc.selectedModels.count; i++) {
            ZKTY_TZAssetModel *model = tzImagePickerVc.selectedModels[i];
            ZKTY_TZImageRequestOperation *operation = [[ZKTY_TZImageRequestOperation alloc] initWithAsset:model.asset completion:^(UIImage * _Nonnull photo, NSDictionary * _Nonnull info, BOOL isDegraded) {
                if (isDegraded) return;
                if (photo) {
                    if (![ZKTY_TZImagePickerConfig sharedInstance].notScaleImage) {
                        photo = [[ZKTY_TZImageManager manager] scaleImage:photo toSize:CGSizeMake(tzImagePickerVc.photoWidth, (int)(tzImagePickerVc.photoWidth * photo.size.height / photo.size.width))];
                    }
...
- (void)callDelegateMethodWithPhotos:(NSArray *)photos assets:(NSArray *)assets infoArr:(NSArray *)infoArr {
    ...
    if (tzImagePickerVc.allowPickingVideo && tzImagePickerVc.maxImagesCount == 1) {
        if ([[ZKTY_TZImageManager manager] isVideo:[assets firstObject]]) {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (((tzImagePickerVc.sortAscendingByModificationDate && indexPath.item >= _models.count) || (!tzImagePickerVc.sortAscendingByModificationDate && indexPath.item == 0)) && _showTakePhotoBtn) {
        ZKTY_TZAssetCameraCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"ZKTY_TZAssetCameraCell" forIndexPath:indexPath];
    ...
    ZKTY_TZAssetCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"ZKTY_TZAssetCell" forIndexPath:indexPath];
    ...
    cell.didSelectPhotoBlock = ^(BOOL isSelected) {
        ...
        ZKTY_TZImagePickerController *tzImagePickerVc = (ZKTY_TZImagePickerController *)strongSelf.navigationController;
        // 1. cancel select / 取消选择
        if (isSelected) {
            strongCell.selectPhotoButton.selected = NO;
            model.isSelected = NO;
            NSArray *selectedModels = [NSArray arrayWithArray:tzImagePickerVc.selectedModels];
            for (ZKTY_TZAssetModel *model_item in selectedModels) {
            ...
            if (tzImagePickerVc.showSelectedIndex || tzImagePickerVc.showPhotoCannotSelectLayer) {
                [[NSNotificationCenter defaultCenter] postNotificationName:@"ZKTY_TZ_PHOTO_PICKER_RELOAD_NOTIFICATION" object:strongSelf.navigationController];
            }
            [UIView showOscillatoryAnimationWithLayer:strongLayer type:ZKTY_TZOscillatoryAnimationToSmaller];
            ...
            if (tzImagePickerVc.selectedModels.count < tzImagePickerVc.maxImagesCount) {
                if (!tzImagePickerVc.allowPreview) {
                    ...
                    if (!tzImagePickerVc.allowPickingMultipleVideo && (model.type == ZKTY_TZAssetModelMediaTypeVideo || model.type == ZKTY_TZAssetModelMediaTypePhotoGif)) {
                        shouldDone = YES;
                    }
                    if (shouldDone) {
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    ZKTY_TZAssetModel *model = _models[index];
    if (model.type == ZKTY_TZAssetModelMediaTypeVideo && !tzImagePickerVc.allowPickingMultipleVideo) {
        if (tzImagePickerVc.selectedModels.count > 0) {
            ...
            ZKTY_TZVideoPlayerController *videoPlayerVc = [[ZKTY_TZVideoPlayerController alloc] init];
            videoPlayerVc.model = model;
            [self.navigationController pushViewController:videoPlayerVc animated:YES];
        }
    } else if (model.type == ZKTY_TZAssetModelMediaTypePhotoGif && tzImagePickerVc.allowPickingGif && !tzImagePickerVc.allowPickingMultipleVideo) {
        if (tzImagePickerVc.selectedModels.count > 0) {
            ZKTY_TZImagePickerController *imagePickerVc = (ZKTY_TZImagePickerController *)self.navigationController;
            [imagePickerVc showAlertWithTitle:[NSBundle tz_localizedStringForKey:@"Can not choose both photo and GIF"]];
        } else {
            ZKTY_TZGifPhotoPreviewController *gifPreviewVc = [[ZKTY_TZGifPhotoPreviewController alloc] init];
...
- (void)takePhoto {
    ...
    if ((authStatus == AVAuthorizationStatusRestricted || authStatus ==AVAuthorizationStatusDenied)) {
        ...
        NSDictionary *infoDict = [ZKTY_TZCommonTools tz_getInfoDictionary];
        // 无权限 做一个友好的提示
...
- (void)pushImagePickerController {
    ...
    if (tzImagePickerVc.allowCameraLocation) {
        __weak typeof(self) weakSelf = self;
        [[ZKTY_TZLocationManager manager] startLocationWithSuccessBlock:^(NSArray<CLLocation *> *locations) {
            __strong typeof(weakSelf) strongSelf = weakSelf;
...
- (void)pushPhotoPrevireViewController:(ZKTY_TZPhotoPreviewController *)photoPreviewVc {
    [self pushPhotoPrevireViewController:photoPreviewVc needCheckSelectedModels:NO];
}

- (void)pushPhotoPrevireViewController:(ZKTY_TZPhotoPreviewController *)photoPreviewVc needCheckSelectedModels:(BOOL)needCheckSelectedModels {
    __weak typeof(self) weakSelf = self;
...
- (void)getSelectedPhotoBytes {
    ...
    if ([[ZKTY_TZImagePickerConfig sharedInstance].preferredLanguage isEqualToString:@"vi"] && self.view.tz_width <= 320) {
        return;
    }
    ZKTY_TZImagePickerController *imagePickerVc = (ZKTY_TZImagePickerController *)self.navigationController;
    [[ZKTY_TZImageManager manager] getPhotosBytesWithArray:imagePickerVc.selectedModels completion:^(NSString *totalBytes) {
...
- (void)checkSelectedModels {
    ...
    for (ZKTY_TZAssetModel *model in selectedModels) {
...
- (void)imagePickerController:(UIImagePickerController*)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    if ([type isEqualToString:@"public.image"]) {
        ...
        if (photo) {
            ...
            [[ZKTY_TZImageManager manager] savePhotoWithImage:photo meta:meta location:self.location completion:^(PHAsset *asset, NSError *error){
        ...
        if (videoUrl) {
            self.isSavingMedia = YES;
            [[ZKTY_TZImageManager manager] saveVideoWithUrl:videoUrl location:self.location completion:^(PHAsset *asset, NSError *error) {
                self.isSavingMedia = NO;
                if (!error && asset) {
                    [self addPHAsset:asset];
                } else {
...
- (void)addPHAsset:(PHAsset *)asset {
    ZKTY_TZAssetModel *assetModel = [[ZKTY_TZImageManager manager] createModelWithAsset:asset];
    ZKTY_TZImagePickerController *tzImagePickerVc = (ZKTY_TZImagePickerController *)self.navigationController;
    [tzImagePickerVc hideProgressHUD];
    if (tzImagePickerVc.sortAscendingByModificationDate) {
    ...
    if (tzImagePickerVc.selectedModels.count < tzImagePickerVc.maxImagesCount) {
        if (assetModel.type == ZKTY_TZAssetModelMediaTypeVideo && !tzImagePickerVc.allowPickingMultipleVideo) {
            // 不能多选视频的情况下，不选中拍摄的视频
        } else {
...
- (void)resetCachedAssets {
    [[ZKTY_TZImageManager manager].cachingImageManager stopCachingImagesForAllAssets];
...
- (NSArray *)assetsAtIndexPaths:(NSArray *)indexPaths {
    ...
    for (NSIndexPath *indexPath in indexPaths) {
        if (indexPath.item < _models.count) {
            ZKTY_TZAssetModel *model = _models[indexPath.item];
            [assets addObject:model.asset];
        }
    }
...
@implementation ZKTY_TZCollectionView
