//
//  ZKTY_TZPhotoPreviewController.m
...
#import "ZKTY_TZImageCropManager.h"

@interface ZKTY_TZPhotoPreviewController ()<UICollectionViewDataSource,UICollectionViewDelegate,UIScrollViewDelegate> {
...
@implementation ZKTY_TZPhotoPreviewController

- (void)viewDidLoad {
    [super viewDidLoad];
    [ZKTY_TZImageManager manager].shouldFixOrientation = YES;
    ZKTY_TZImagePickerController *_tzImagePickerVc = (ZKTY_TZImagePickerController *)self.navigationController;
...
- (void)configCollectionView {
    ...
    [_collectionView registerClass:[ZKTY_TZPhotoPreviewCell class] forCellWithReuseIdentifier:@"ZKTY_TZPhotoPreviewCell"];
    [_collectionView registerClass:[ZKTY_TZVideoPreviewCell class] forCellWithReuseIdentifier:@"ZKTY_TZVideoPreviewCell"];
    [_collectionView registerClass:[ZKTY_TZGifPreviewCell class] forCellWithReuseIdentifier:@"ZKTY_TZGifPreviewCell"];
...
- (void)configCropView {
    ...
    if (_tzImagePickerVc.maxImagesCount <= 1 && _tzImagePickerVc.allowCrop && _tzImagePickerVc.allowPickingImage) {
        ...
        [ZKTY_TZImageCropManager overlayClippingWithView:_cropBgView cropRect:_tzImagePickerVc.cropRect containerView:self.view needCircleCrop:_tzImagePickerVc.needCircleCrop];
...
- (void)viewDidLayoutSubviews {
    ...
    CGFloat statusBarHeight = isFullScreen ? [ZKTY_TZCommonTools tz_statusBarHeight] : 0;
...
- (void)select:(UIButton *)selectButton {
    ZKTY_TZImagePickerController *_tzImagePickerVc = (ZKTY_TZImagePickerController *)self.navigationController;
    ZKTY_TZAssetModel *model = _models[self.currentIndex];
    if (!selectButton.isSelected) {
        ...
        if (_tzImagePickerVc.selectedModels.count >= _tzImagePickerVc.maxImagesCount) {
            ...
            if (model.type == ZKTY_TZAssetModelMediaTypeVideo && !_tzImagePickerVc.allowPickingMultipleVideo) {
        ...
        for (ZKTY_TZAssetModel *model_item in selectedModels) {
            if ([model.asset.localIdentifier isEqualToString:model_item.asset.localIdentifier]) {
                ...
                for (NSInteger i = 0; i < selectedModelsTmp.count; i++) {
                    ZKTY_TZAssetModel *model = selectedModelsTmp[i];
    ...
    [self refreshNaviBarAndBottomBarState];
    if (model.isSelected) {
        [UIView showOscillatoryAnimationWithLayer:selectButton.imageView.layer type:ZKTY_TZOscillatoryAnimationToBigger];
    }
    [UIView showOscillatoryAnimationWithLayer:_numberImageView.layer type:ZKTY_TZOscillatoryAnimationToSmaller];
}

- (void)backButtonClick {
    if (self.navigationController.childViewControllers.count < 2) {
        [self.navigationController dismissViewControllerAnimated:YES completion:nil];
        if ([self.navigationController isKindOfClass: [ZKTY_TZImagePickerController class]]) {
            ZKTY_TZImagePickerController *nav = (ZKTY_TZImagePickerController *)self.navigationController;
...
- (void)doneButtonClick {
    ...
    ZKTY_TZPhotoPreviewCell *cell = (ZKTY_TZPhotoPreviewCell *)[_collectionView cellForItemAtIndexPath:indexPath];
    if (_tzImagePickerVc.allowCrop && [cell isKindOfClass:[ZKTY_TZPhotoPreviewCell class]]) { // 裁剪状态
        _doneButton.enabled = NO;
        [_tzImagePickerVc showProgressHUD];
        UIImage *cropedImage = [ZKTY_TZImageCropManager cropImageView:cell.previewView.imageView toRect:_tzImagePickerVc.cropRect zoomScale:cell.previewView.scrollView.zoomScale containerView:self.view];
        if (_tzImagePickerVc.needCircleCrop) {
            cropedImage = [ZKTY_TZImageCropManager circularClipImage:cropedImage];
        }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    ZKTY_TZAssetPreviewCell *cell;
    __weak typeof(self) weakSelf = self;
    if (_tzImagePickerVc.allowPickingMultipleVideo && model.type == ZKTY_TZAssetModelMediaTypeVideo) {
        cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"ZKTY_TZVideoPreviewCell" forIndexPath:indexPath];
        ZKTY_TZVideoPreviewCell *currentCell = (ZKTY_TZVideoPreviewCell *)cell;
    ...
    } else if (_tzImagePickerVc.allowPickingMultipleVideo && model.type == ZKTY_TZAssetModelMediaTypePhotoGif && _tzImagePickerVc.allowPickingGif) {
        cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"ZKTY_TZGifPreviewCell" forIndexPath:indexPath];
        ZKTY_TZGifPreviewCell *currentCell = (ZKTY_TZGifPreviewCell *)cell;
        currentCell.previewView.iCloudSyncFailedHandle = ^(id asset, BOOL isSyncFailed) {
            model.iCloudFailed = isSyncFailed;
            [weakSelf didICloudSyncStatusChanged:model];
        };
    } else {
        cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"ZKTY_TZPhotoPreviewCell" forIndexPath:indexPath];
        ZKTY_TZPhotoPreviewCell *photoPreviewCell = (ZKTY_TZPhotoPreviewCell *)cell;
...
- (void)collectionView:(UICollectionView *)collectionView willDisplayCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath {
    if ([cell isKindOfClass:[ZKTY_TZPhotoPreviewCell class]]) {
        [(ZKTY_TZPhotoPreviewCell *)cell recoverSubviews];
    }
}
...
- (void)collectionView:(UICollectionView *)collectionView didEndDisplayingCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath {
    if ([cell isKindOfClass:[ZKTY_TZPhotoPreviewCell class]]) {
        [(ZKTY_TZPhotoPreviewCell *)cell recoverSubviews];
    } else if ([cell isKindOfClass:[ZKTY_TZVideoPreviewCell class]]) {
        ZKTY_TZVideoPreviewCell *videoCell = (ZKTY_TZVideoPreviewCell *)cell;
...
- (void)refreshNaviBarAndBottomBarState {
    ...
    // 如果正在预览的是视频，隐藏原图按钮
    if (!_isHideNaviBar) {
        if (model.type == ZKTY_TZAssetModelMediaTypeVideo) {
    ...
    if (![[ZKTY_TZImageManager manager] isPhotoSelectableWithAsset:model.asset]) {
...
- (void)didICloudSyncStatusChanged:(ZKTY_TZAssetModel *)model{
    dispatch_async(dispatch_get_main_queue(), ^{
        ZKTY_TZImagePickerController *_tzImagePickerVc = (ZKTY_TZImagePickerController *)self.navigationController;
        // onlyReturnAsset为NO时,依赖ZKTY_TZ返回大图,所以需要有iCloud同步失败的提示,并且不能选择,
        if (_tzImagePickerVc.onlyReturnAsset) {
            return;
        }
        ZKTY_TZAssetModel *currentModel = self.models[self.currentIndex];
...
- (void)showPhotoBytes {
    [[ZKTY_TZImageManager manager] getPhotosBytesWithArray:@[_models[self.currentIndex]] completion:^(NSString *totalBytes) {
        self->_originalPhotoLabel.text = [NSString stringWithFormat:@"(%@)",totalBytes];
    }];
}

- (NSInteger)currentIndex {
    return [ZKTY_TZCommonTools tz_isRightToLeftLayout] ? self.models.count - _currentIndex - 1 : _currentIndex;
}
