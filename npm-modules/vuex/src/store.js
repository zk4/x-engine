  ...
  constructor (options = {}) {
    ...
    if (native_state) {
      if ('modules' in options) {
        const state = JSON.parse(native_state)
        Object.keys(options.modules).forEach(key => {
          let optionsState = options.modules[key].state
          options.modules[key].state = {
            ...optionsState,
            ...state[key]
          }
        })
    ...
    if (!xengine.platform.isPc) {
      this._subscribers.push((mutation, state) => {
      ...
      xengine.broadcastOn((type, payload) => {
        if (type === '@@VUEX_STORE_EVENT') {
          ...
          Object.keys(this._vm._data.$$state).forEach(key => {
            this._vm._data.$$state[key] = state[key];
          })
        }
      });
    }
